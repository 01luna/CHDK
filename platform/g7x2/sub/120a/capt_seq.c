/*
 * capt_seq.c - auto-generated by CHDK code_gen2.
 */

#include "lolevel.h"
#include "platform.h"
#include "core.h"

// TODO:

#define USE_STUBS_NRFLAG 1
#define NR_AUTO (-1) // default value if NRTBL.SetDarkSubType not used is -1 (0 probably works the same), set to enable auto

#include "../../../generic/capt_seq.c"

char *current_raw_addr;
char *current_raw_alt_addr;

void captseq_get_raw_addr()
{
    extern char* raw_buffer_table[];
    extern int raw_buffer_index;
    current_raw_addr = raw_buffer_table[raw_buffer_index];
    current_raw_alt_addr = raw_buffer_table[(raw_buffer_index+1)&0xF];   // 16 buffers, select next one as alt
}

void clear_current_raw_addr(void)
{
    current_raw_addr = NULL;
}

/*************************************************************/
//** capt_seq_task @ 0xE0032F57, length=178 
void __attribute__((naked,noinline)) capt_seq_task() {
asm volatile (
            "    push    {r3, r4, r5, r6, r7, lr}\n"
            "    movs    r6, #0\n"
            "    ldr     r4, =0x00057d54\n"
            "    ldr     r5, =0x0000c098\n"
            "loc_e0032f5e:\n"
            "    movs    r2, #0\n"
            "    mov     r1, sp\n"
            "    ldr     r0, [r5, #8]\n"
            "    bl      sub_dffc9de0\n" // ReceiveMessageQueue
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e0032f80\n"
            "    movw    r2, #0x435\n"
            "    ldr     r1, =0xe0032b9c\n" //  *"SsShootTask.c"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "    bl      _ExitTask\n"
            "    pop     {r3, r4, r5, r6, r7, pc}\n"
            "loc_e0032f80:\n"
            "    ldr     r0, [sp]\n"
            "    ldr     r0, [r0]\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e0032f98\n"
            "    cmp     r0, #0x2b\n"
            "    beq     loc_e0032f98\n"
            "    cmp     r0, #0x1f\n"
            "    beq     loc_e0032f98\n"
            "    cmp     r0, #0x23\n"
            "    beq     loc_e0032f98\n"
            "    bl      sub_e005333c\n"
            "loc_e0032f98:\n"
            "    ldr     r0, [sp]\n"
            "    ldr     r1, [r0]\n"
            "    cmp     r1, #0x33\n"
            "    bhs     loc_e0033080\n"
            "    tbb     [pc, r1]\n" // (jumptable r1 51 elements)
            "branchtable_e0032fa4:\n"
            "    .byte((loc_e0032fd8 - branchtable_e0032fa4) / 2)\n" // (case 0)
            "    .byte((loc_e0032fee - branchtable_e0032fa4) / 2)\n" // (case 1)
            "    .byte((loc_e0033008 - branchtable_e0032fa4) / 2)\n" // (case 2)
            "    .byte((loc_e0033016 - branchtable_e0032fa4) / 2)\n" // (case 3)
            "    .byte((loc_e0033010 - branchtable_e0032fa4) / 2)\n" // (case 4)
            "    .byte((loc_e0033020 - branchtable_e0032fa4) / 2)\n" // (case 5)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 6)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 7)
            "    .byte((loc_e0033026 - branchtable_e0032fa4) / 2)\n" // (case 8)
            "    .byte((loc_e0033036 - branchtable_e0032fa4) / 2)\n" // (case 9)
            "    .byte((loc_e003303e - branchtable_e0032fa4) / 2)\n" // (case 10)
            "    .byte((loc_e0033044 - branchtable_e0032fa4) / 2)\n" // (case 11)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 12)
            "    .byte((loc_e003304c - branchtable_e0032fa4) / 2)\n" // (case 13)
            "    .byte((loc_e0033052 - branchtable_e0032fa4) / 2)\n" // (case 14)
            "    .byte((loc_e0033058 - branchtable_e0032fa4) / 2)\n" // (case 15)
            "    .byte((loc_e003305e - branchtable_e0032fa4) / 2)\n" // (case 16)
            "    .byte((loc_e0033064 - branchtable_e0032fa4) / 2)\n" // (case 17)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 18)
            "    .byte((loc_e003306a - branchtable_e0032fa4) / 2)\n" // (case 19)
            "    .byte((loc_e0033070 - branchtable_e0032fa4) / 2)\n" // (case 20)
            "    .byte((loc_e0033076 - branchtable_e0032fa4) / 2)\n" // (case 21)
            "    .byte((loc_e003307a - branchtable_e0032fa4) / 2)\n" // (case 22)
            "    .byte((loc_e0033082 - branchtable_e0032fa4) / 2)\n" // (case 23)
            "    .byte((loc_e0033088 - branchtable_e0032fa4) / 2)\n" // (case 24)
            "    .byte((loc_e003308e - branchtable_e0032fa4) / 2)\n" // (case 25)
            "    .byte((loc_e0033094 - branchtable_e0032fa4) / 2)\n" // (case 26)
            "    .byte((loc_e003309a - branchtable_e0032fa4) / 2)\n" // (case 27)
            "    .byte((loc_e00330a2 - branchtable_e0032fa4) / 2)\n" // (case 28)
            "    .byte((loc_e00330a8 - branchtable_e0032fa4) / 2)\n" // (case 29)
            "    .byte((loc_e00330ae - branchtable_e0032fa4) / 2)\n" // (case 30)
            "    .byte((loc_e00330b2 - branchtable_e0032fa4) / 2)\n" // (case 31)
            "    .byte((loc_e00330ba - branchtable_e0032fa4) / 2)\n" // (case 32)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 33)
            "    .byte((loc_e00330c0 - branchtable_e0032fa4) / 2)\n" // (case 34)
            "    .byte((loc_e00330c6 - branchtable_e0032fa4) / 2)\n" // (case 35)
            "    .byte((loc_e00330cc - branchtable_e0032fa4) / 2)\n" // (case 36)
            "    .byte((loc_e00330d2 - branchtable_e0032fa4) / 2)\n" // (case 37)
            "    .byte((loc_e00330d8 - branchtable_e0032fa4) / 2)\n" // (case 38)
            "    .byte((loc_e00330e0 - branchtable_e0032fa4) / 2)\n" // (case 39)
            "    .byte((loc_e00330e6 - branchtable_e0032fa4) / 2)\n" // (case 40)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 41)
            "    .byte((loc_e003311c - branchtable_e0032fa4) / 2)\n" // (case 42)
            "    .byte((loc_e0033122 - branchtable_e0032fa4) / 2)\n" // (case 43)
            "    .byte((loc_e0033106 - branchtable_e0032fa4) / 2)\n" // (case 44)
            "    .byte((loc_e0033148 - branchtable_e0032fa4) / 2)\n" // (case 45)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 46)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 47)
            "    .byte((loc_e003313a - branchtable_e0032fa4) / 2)\n" // (case 48)
            "    .byte((loc_e003312e - branchtable_e0032fa4) / 2)\n" // (case 49)
            "    .byte((loc_e0033134 - branchtable_e0032fa4) / 2)\n" // (case 50)
            ".align 1\n"
            "loc_e0032fd8:\n"
            "    ldr     r0, [r0, #0xc]\n"
            "    bl      sub_e0039274\n"
            "    bl      shooting_expo_param_override\n" // +
            "    bl      sub_e004f1e4\n"
            "    ldr     r0, [r4, #0x28]\n"
            "    cmp     r0, #0\n"
            "    beq     loc_e0032fec\n"
            "    bl      sub_e00464ae_my\n" // -> nr?, remote hook, raw hook (quick, no half press) (was sub_e00464ae)
            "loc_e0032fec:\n"
            "    b       loc_e0033148\n"
            "loc_e0032fee:\n"
            "    ldr     r0, [r0, #0x10]\n"
            "    bl      sub_e004644a_my\n" // -> nr?, remote hook, raw hook (was sub_e004644a)
            "    b       loc_e0033148\n"
            "loc_e0033008:\n"
            "    movs    r0, #1\n"
            "    bl      sub_e00395a2\n"
            "    b       loc_e0033148\n"
            "loc_e0033010:\n"
            "    bl      sub_e0038f62\n"
            "    b       loc_e003301c\n"
            "loc_e0033016:\n"
            "    ldr     r0, [r0, #0xc]\n"
            "    bl      sub_e003925c\n"
            "loc_e003301c:\n"
            "    str     r6, [r4, #0x28]\n"
            "    b       loc_e0033148\n"
            "loc_e0033020:\n"
            "    bl      sub_e0039262\n"
            "    b       loc_e0033148\n"
            "loc_e0033026:\n"
            "    bl      sub_e0039532\n"
            "    bl      sub_e004f1e4\n"
            "    movs    r0, #0\n"
            "    bl      sub_e0297bcc\n"
            "    b       loc_e0033148\n"
            "loc_e0033036:\n"
            "    ldr     r0, [r4, #0x54]\n"
            "    bl      sub_e0039b92\n"
            "    b       loc_e0033148\n"
            "loc_e003303e:\n"
            "    bl      sub_e0039dfc\n"
            "    b       loc_e0033148\n"
            "loc_e0033044:\n"
            "    ldr     r0, [r0, #0xc]\n"
            "    bl      sub_e0039e48\n"
            "    b       loc_e0033148\n"
            "loc_e003304c:\n"
            "    bl      sub_e003a030\n"
            "    b       loc_e0033148\n"
            "loc_e0033052:\n"
            "    bl      sub_e003a41c\n"
            "    b       loc_e0033148\n"
            "loc_e0033058:\n"
            "    bl      sub_e003a4b0\n"
            "    b       loc_e0033148\n"
            "loc_e003305e:\n"
            "    bl      sub_e003bfc0\n"
            "    b       loc_e0033148\n"
            "loc_e0033064:\n"
            "    bl      sub_e003c15c\n"
            "    b       loc_e0033148\n"
            "loc_e003306a:\n"
            "    bl      sub_e003c1f8\n"
            "    b       loc_e0033148\n"
            "loc_e0033070:\n"
            "    bl      sub_e003c274\n"
            "    b       loc_e0033148\n"
            "loc_e0033076:\n"
            "    movs    r0, #0\n"
            "    b       loc_e003309c\n"
            "loc_e003307a:\n"
            "    bl      sub_e003c57a\n"
            "    b       loc_e0033148\n"
            "loc_e0033080:\n"
            "    b       loc_e003313a\n"
            "loc_e0033082:\n"
            "    bl      sub_e003c5da\n"
            "    b       loc_e0033148\n"
            "loc_e0033088:\n"
            "    bl      sub_e003c5de\n"
            "    b       loc_e0033148\n"
            "loc_e003308e:\n"
            "    bl      sub_e003c5f0\n"
            "    b       loc_e0033148\n"
            "loc_e0033094:\n"
            "    bl      sub_e003c674\n"
            "    b       loc_e0033148\n"
            "loc_e003309a:\n"
            "    movs    r0, #1\n"
            "loc_e003309c:\n"
            "    bl      sub_e003c458\n"
            "    b       loc_e0033148\n"
            "loc_e00330a2:\n"
            "    bl      sub_e003c720\n"
            "    b       loc_e0033148\n"
            "loc_e00330a8:\n"
            "    bl      sub_e0039672\n"
            "    b       loc_e0033148\n"
            "loc_e00330ae:\n"
            "    movs    r0, #0\n"
            "    b       loc_e00330b4\n"
            "loc_e00330b2:\n"
            "    ldr     r0, [r0, #0xc]\n"
            "loc_e00330b4:\n"
            "    bl      sub_e00396f2\n"
            "    b       loc_e0033148\n"
            "loc_e00330ba:\n"
            "    bl      sub_e003c37c\n"
            "    b       loc_e0033148\n"
            "loc_e00330c0:\n"
            "    bl      sub_e003c3e8\n"
            "    b       loc_e0033148\n"
            "loc_e00330c6:\n"
            "    bl      sub_e0046668\n"
            "    b       loc_e0033148\n"
            "loc_e00330cc:\n"
            "    bl      sub_e003afdc\n"
            "    b       loc_e0033148\n"
            "loc_e00330d2:\n"
            "    bl      sub_e003b096\n"
            "    b       loc_e0033148\n"
            "loc_e00330d8:\n"
            "    ldr     r0, [r0, #0xc]\n"
            "    bl      sub_e003c7d8\n"
            "    b       loc_e0033148\n"
            "loc_e00330e0:\n"
            "    bl      sub_e003c840\n"
            "    b       loc_e0033148\n"
            "loc_e00330e6:\n"
            "    bl      sub_e00361dc\n"
            "    ldrh.w  r0, [r4, #0x1a0]\n"
            "    cmp     r0, #4\n"
            "    beq     loc_e00330fc\n"
            "    ldrh    r0, [r4]\n"
            "    sub.w   r1, r0, #0x8200\n"
            "    subs    r1, #0x37\n"
            "    bne     loc_e0033148\n"
            "loc_e00330fc:\n"
            "    bl      sub_e003b096\n"
            "    bl      sub_e003b410\n"
            "    b       loc_e0033148\n"
            "loc_e0033106:\n"
            "    bl      sub_e003a6ba\n"
            "    movs    r0, #1\n"
            "    bl      sub_e0035df4\n"
            "    cbnz    r0, loc_e0033148\n"
            "    movs    r1, #0\n"
            "    movs    r0, #6\n"
            "    bl      sub_e003c978\n"
            "    b       loc_e0033148\n"
            "loc_e003311c:\n"
            "    movs    r2, #0\n"
            "    movs    r1, #0x11\n"
            "    b       loc_e0033126\n"
            "loc_e0033122:\n"
            "    movs    r2, #0\n"
            "    movs    r1, #0x10\n"
            "loc_e0033126:\n"
            "    movs    r0, #0\n"
            "    bl      sub_e004ff4a\n"
            "    b       loc_e0033148\n"
            "loc_e003312e:\n"
            "    bl      sub_e003d212\n"
            "    b       loc_e0033148\n"
            "loc_e0033134:\n"
            "    bl      sub_e003d214\n"
            "    b       loc_e0033148\n"
            "loc_e003313a:\n"
            "    movw    r2, #0x566\n"
            "    ldr     r1, =0xe0032b9c\n" //  *"SsShootTask.c"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "loc_e0033148:\n"
            "    bl      capt_seq_hook_set_nr\n" //  dark frame control
            "    ldr     r0, [sp]\n"
            "    ldr     r1, [r0, #4]\n"
            "    ldr     r0, [r5, #4]\n"
            "    bl      _SetEventFlag\n"
            "    ldr     r7, [sp]\n"
            "    ldr     r0, [r7, #8]\n"
            "    cbnz    r0, loc_e0033164\n"
            "    movs    r2, #0xff\n"
            "    ldr     r1, =0xe0032bb8\n" //  *"SsShootTask.c"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "loc_e0033164:\n"
            "    str     r6, [r7, #8]\n"
            "    b       loc_e0032f5e\n"
);
}

/*************************************************************/
//** sub_e00464ae_my @ 0xE00464AF, length=46 
void __attribute__((naked,noinline)) sub_e00464ae_my() {
asm volatile (
            "    push    {r3, r4, r5, r6, r7, lr}\n"
            "    bl      sub_e0050788\n"
            "    mov     r6, r0\n"
            "    movs    r0, #0xc\n"
            "    bl      sub_e02f079c\n"
            "    ldr     r5, =0x00012164\n"
            "    lsls    r0, r0, #0x1f\n"
            "    mov.w   r4, #1\n"
            "    bne     loc_e0046522\n"
            "    movs    r2, #2\n"
            "    mov     r1, sp\n"
            "    movw    r0, #0x113\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00464e2\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x1b4\n"
            "    ldr     r1, =0xe00465a8\n" //  *"SsCaptureCtrl.c"
            "    bl      _DebugAssert\n"
            "loc_e00464e2:\n"
            "    ldrsh.w r0, [sp]\n"
            "    bl      sub_e049142e\n"
            "    bl      sub_e04911de\n"
            "    cmp     r0, #1\n"
            "    bls     loc_e0046526\n"
            "    movs    r0, #0\n"
            "    bl      sub_e04912fc\n"
            "    bl      sub_e03817dc\n"
            "    bl      sub_e0039268\n"
            "    mov     r0, r6\n"
            "    bl      sub_e0051f14\n"
            "    movs    r2, #4\n"
            "    movs    r0, #0x46\n"
            "    add.w   r1, r6, #8\n"
            "    bl      _SetPropertyCase\n"
            "    mov     r0, r6\n"
            "    bl      sub_e004b410\n"
            "    mov     r0, r6\n"
            "    bl      sub_e005348c_my\n" // Patched (was sub_e005348c)
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e0046524\n" //  return
            "loc_e0046522:\n"
            "    str     r4, [r5]\n"
            "loc_e0046524:\n"
            "    pop     {r3, r4, r5, r6, r7, pc}\n"
            "loc_e0046526:\n"
            "    str     r4, [r5]\n"
            "    bl      sub_e004fc16\n"
            "    bl      sub_e02f07dc\n"
            "    pop     {r3, r4, r5, r6, r7, pc}\n"
);
}

/*************************************************************/
//** sub_e004644a_my @ 0xE004644B, length=41 
void __attribute__((naked,noinline)) sub_e004644a_my() {
asm volatile (
            "    push    {r4, r5, r6, lr}\n"
            "    mov     r6, r0\n"
            "    ldr     r5, =0x00057d54\n"
            "    movs    r4, #0\n"
            "    mov.w   r3, #0x120\n"
            "    ldr     r0, [r5, #0x28]\n"
            "    cbz     r0, loc_e0046482\n"
            "    ldr.w   r2, [r6, #0x138]\n"
            "    ldr     r1, =0xe00465c0\n" //  *"CaptCtrl:Quick(%d)"
            "    mov     r0, r3\n"
            "    bl      sub_e036b640\n" // LogCameraEvent
            "    ldr     r0, =0x00012164\n"
            "    ldr     r0, [r0]\n"
            "    cbz     r0, loc_e004646e\n"
            "    movs    r4, #0x1d\n"
            "loc_e004646e:\n"
            "    movs    r1, #2\n"
            "    mov     r2, r6\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004ff4a\n"
            "    mov     r1, r4\n"
            "    mov     r0, r6\n"
            "    bl      sub_e00531c8\n"
            "    b       loc_e00464a8\n"
            "loc_e0046482:\n"
            "    ldr     r1, =0xe00465d8\n" //  *"CaptCtrl:ExecCapt"
            "    mov     r0, r3\n"
            "    bl      sub_e036b640\n" // LogCameraEvent
            "    mov     r0, r6\n"
            "    bl      sub_e004626a_my\n" // Patched (was sub_e004626a)
            "    mov     r4, r0\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00464a8\n"
            "    movs    r1, #2\n"
            "    mov     r2, r6\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004ff4a\n"
            "    mov     r1, r4\n"
            "    mov     r0, r6\n"
            "    bl      sub_e00532fe\n"
            "loc_e00464a8:\n"
            "    movs    r0, #0\n"
            "    str     r0, [r5, #0x28]\n"
            "    pop     {r4, r5, r6, pc}\n"
);
}

/*************************************************************/
//** sub_e004626a_my @ 0xE004626B, length=178 
void __attribute__((naked,noinline)) sub_e004626a_my() {
asm volatile (
            "    push.w  {r2, r3, r4, r5, r6, r7, r8, lr}\n"
            "    mov     r4, r0\n"
            "    bl      sub_e0051f14\n"
            "    ldr     r6, =0x00057d54\n"
            "    ldr.w   r0, [r6, #0x108]\n"
            "    cbz     r0, loc_e0046284\n"
            "    ldr     r0, [r4, #0xc]\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e0046292\n"
            "    b       loc_e00462a2\n"
            "loc_e0046284:\n"
            "    ldrh.w  r0, [r6, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e0046292\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00462a2\n"
            "loc_e0046292:\n"
            "    movs    r0, #0xc\n"
            "    bl      sub_e02f079c\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00462a2\n"
            "loc_e004629c:\n"
            "    bl      sub_e004fc16\n"
            "    b       loc_e0046302\n" //  return 0x1
            "loc_e00462a2:\n"
            "    ldr.w   r0, [r6, #0xec]\n"
            "    cbz     r0, loc_e00462c2\n"
            "    ldrh.w  r0, [r6, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e00462b6\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00462f2\n"
            "loc_e00462b6:\n"
            "    ldr.w   r0, [r6, #0x108]\n"
            "    cbz     r0, loc_e00462c2\n"
            "    ldr     r0, [r4, #0xc]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00462f2\n"
            "loc_e00462c2:\n"
            "    movs    r2, #2\n"
            "    mov     r1, sp\n"
            "    movw    r0, #0x113\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00462dc\n"
            "    movs    r2, #0xe5\n"
            "    movs    r0, #0\n"
            "    ldr     r1, =0xe00465a8\n" //  *"SsCaptureCtrl.c"
            "    bl      _DebugAssert\n"
            "loc_e00462dc:\n"
            "    ldrsh.w r0, [sp]\n"
            "    bl      sub_e049142e\n"
            "    bl      sub_e04911de\n"
            "    cmp     r0, #1\n"
            "    bls     loc_e00462fa\n"
            "    movs    r0, #0\n"
            "    bl      sub_e04912fc\n"
            "loc_e00462f2:\n"
            "    ldr.w   r0, [r6, #0x1cc]\n"
            "    cbnz    r0, loc_e004630e\n"
            "    b       loc_e0046308\n"
            "loc_e00462fa:\n"
            "    bl      sub_e004fc16\n"
            "    bl      sub_e02f07dc\n"
            "loc_e0046302:\n"
            "    movs    r0, #1\n"
            "loc_e0046304:\n"
            "    pop.w   {r2, r3, r4, r5, r6, r7, r8, pc}\n"
            "loc_e0046308:\n"
            "    ldr.w   r0, [r6, #0x1c8]\n"
            "    cbz     r0, loc_e0046326\n"
            "loc_e004630e:\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e0046320\n"
            "    movs    r0, #1\n"
            "    bl      sub_e0035df4\n"
            "    lsls    r0, r0, #0x1f\n"
            "    bne     loc_e004629c\n"
            "    b       loc_e0046326\n"
            "loc_e0046320:\n"
            "    movs    r0, #0x64\n"
            "    bl      _SleepTask\n"
            "loc_e0046326:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e00396b8\n"
            "    lsls    r1, r0, #0x1f\n"
            "    bne     loc_e0046304\n" //  return
            "    ldr     r7, =0x00057f68\n"
            "    ldr     r0, [r7]\n"
            "    cbz     r0, loc_e004633c\n"
            "    mov     r0, r4\n"
            "    bl      sub_e005341c\n"
            "loc_e004633c:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004965e\n"
            "    mov     r5, r0\n"
            "    lsls    r0, r0, #0x1f\n"
            "    bne     loc_e0046446\n"
            "    bl      sub_e03817dc\n"
            "    bl      sub_e0039268\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004b410\n"
            "    ldr.w   r0, [r6, #0x12c]\n"
            "    cbnz    r0, loc_e004636a\n"
            "    ldrh.w  r0, [r6, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e004636a\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e0046370\n"
            "loc_e004636a:\n"
            "    movs    r0, #2\n"
            "    bl      sub_e003e64e\n"
            "loc_e0046370:\n"
            "    ldr.w   r0, [r6, #0x9c]\n"
            "    cbz     r0, loc_e00463d4\n"
            "    ldrh.w  r0, [r6, #0x19e]\n"
            "    mov.w   r8, #0x800\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e004639e\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bls     loc_e004639e\n"
            "    bl      sub_e005157e\n"
            "    mov.w   r1, #0x13a\n"
            "    ldr     r3, =0xe00465a8\n" //  *"SsCaptureCtrl.c"
            "    str     r1, [sp]\n"
            "    movw    r2, #0x3a98\n"
            "    mov     r1, r8\n"
            "    bl      sub_e02f0954\n"
            "loc_e004639e:\n"
            "    movs    r2, #4\n"
            "    add     r1, sp, #4\n"
            "    movw    r0, #0x18b\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00463ba\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x13e\n"
            "    ldr     r1, =0xe00465a8\n" //  *"SsCaptureCtrl.c"
            "    bl      _DebugAssert\n"
            "loc_e00463ba:\n"
            "    ldr     r0, [sp, #4]\n"
            "    cbz     r0, loc_e00463e2\n"
            "    bl      sub_e005157e\n"
            "    mov     r1, r8\n"
            "    bl      sub_dffc9966\n" // ClearEventFlag
            "    ldr     r2, =0xe0046259\n"
            "    mov     r3, r8\n"
            "    ldr     r0, [sp, #4]\n"
            "    mov     r1, r2\n"
            "    bl      sub_e0370142\n"
            "loc_e00463d4:\n"
            "    ldr.w   r0, [r6, #0xa4]\n"
            "    cbz     r0, loc_e00463ee\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0053cae\n"
            "    b       loc_e0046432\n"
            "loc_e00463e2:\n"
            "    bl      sub_e005157e\n"
            "    mov     r1, r8\n"
            "    bl      _SetEventFlag\n"
            "    b       loc_e00463d4\n"
            "loc_e00463ee:\n"
            "    ldr.w   r0, [r6, #0xa8]\n"
            "    cbz     r0, loc_e00463fc\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0053940\n"
            "    b       loc_e0046432\n"
            "loc_e00463fc:\n"
            "    ldr.w   r0, [r6, #0xac]\n"
            "    cbz     r0, loc_e004640a\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0053f92\n"
            "    b       loc_e0046432\n"
            "loc_e004640a:\n"
            "    ldr.w   r0, [r6, #0xb0]\n"
            "    cbz     r0, loc_e004641c\n"
            "    ldr     r0, [r7]\n"
            "    cbz     r0, loc_e004641c\n"
            "    mov     r0, r4\n"
            "    bl      sub_e005424e\n"
            "    b       loc_e0046432\n"
            "loc_e004641c:\n"
            "    ldr.w   r0, [r6, #0xb4]\n"
            "    cmp     r0, #0\n"
            "    mov     r0, r4\n"
            "    beq     loc_e004642e\n"
            "    bl      sub_e005452a\n"
            "    b       loc_e0046432\n"
            "    b       loc_e0046446\n"
            "loc_e004642e:\n"
            "    bl      sub_e005348c_my\n" // Patched (was sub_e005348c)
            "loc_e0046432:\n"
            "    ldr.w   r0, [r6, #0x1cc]\n"
            "    cbnz    r0, loc_e004643e\n"
            "    ldr.w   r0, [r6, #0x1c8]\n"
            "    cbz     r0, loc_e0046446\n"
            "loc_e004643e:\n"
            "    movs    r1, #0\n"
            "    movs    r0, #1\n"
            "    bl      sub_e0035e3c\n"
            "loc_e0046446:\n"
            "    mov     r0, r5\n"
            "    b       loc_e0046304\n" //  return
);
}

/*************************************************************/
//** sub_e005348c_my @ 0xE005348D, length=293 
void __attribute__((naked,noinline)) sub_e005348c_my() {
asm volatile (
            "    push.w  {r0, r1, r2, r3, r4, r5, r6, r7, r8, sb, sl, lr}\n"
            "    mov     r4, r0\n"
            "    bl      captseq_get_raw_addr\n"  // +
            "    ldr     r7, =0x00057d54\n"
            "    ldr.w   r0, [r7, #0x104]\n"
            "    cbz     r0, loc_e00534a8\n"
            "    ldrh.w  r0, [r7, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e00534a8\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00534ae\n"
            "loc_e00534a8:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0051e2a\n"
            "loc_e00534ae:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0051fdc\n"
            "    ldr.w   r0, [r7, #0x168]\n"
            "    cbz     r0, loc_e00534c8\n"
            "    ldrh.w  r0, [r7, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e00534c8\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00534d2\n"
            "loc_e00534c8:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004b35c\n"
            "    bl      sub_e003ccf0\n"
            "loc_e00534d2:\n"
            "    ldr.w   r0, [r7, #0x98]\n"
            "    cbnz    r0, loc_e00534f0\n"
            "    ldrh.w  r0, [r7, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e00534e6\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e00534f0\n"
            "loc_e00534e6:\n"
            "    movs    r0, #1\n"
            "    bl      sub_e0039694\n"
            "    bl      sub_e003c8ca\n"
            "loc_e00534f0:\n"
            "    movs    r2, #4\n"
            "    add     r1, sp, #8\n"
            "    mov.w   r0, #0x13a\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e005350c\n"
            "    movs    r0, #0\n"
            "    movw    r2, #0x191\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e005350c:\n"
            "    ldr     r0, [sp, #8]\n"
            "    ubfx    r0, r0, #8, #8\n"
            "    cmp     r0, #6\n"
            "    beq     loc_e0053586\n"
            "    mov     r1, r4\n"
            "    ldr     r0, =0xe004b013\n"
            "loc_e005351a:\n"
            "    bl      sub_e0066cf0\n"
            "    movs    r2, #2\n"
            "    add     r1, sp, #0xc\n"
            "    movw    r0, #0x115\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e005353a\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x19a\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e005353a:\n"
            "    ldr.w   r0, [r7, #0x104]\n"
            "    cbz     r0, loc_e005356e\n"
            "    ldrh.w  r0, [r7, #0x19e]\n"
            "    cmp     r0, #3\n"
            "    beq     loc_e005354e\n"
            "    ldr     r0, [r4, #8]\n"
            "    cmp     r0, #1\n"
            "    bhi     loc_e005356e\n"
            "loc_e005354e:\n"
            "    movs    r0, #1\n"
            "    movw    r5, #0x169\n"
            "    strh.w  r0, [sp]\n"
            "    movs    r2, #2\n"
            "    mov     r1, sp\n"
            "    mov     r0, r5\n"
            "    bl      _SetPropertyCase\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r3, #2\n"
            "    mov     r2, sp\n"
            "    mov     r1, r5\n"
            "    bl      sub_e036b28a\n"
            "loc_e005356e:\n"
            "    ldr.w   r0, [r7, #0x188]\n"
            "    cbz     r0, loc_e0053578\n"
            "    bl      sub_e004b294\n"
            "loc_e0053578:\n"
            "    movs    r0, #0\n"
            "    bl      sub_e0035dec\n" //  return
            "    ldr     r0, =0x00057f68\n"
            "    ldr     r0, [r0]\n"
            "    cbz     r0, loc_e005358c\n"
            "    b       loc_e0053592\n"
            "loc_e0053586:\n"
            "    ldr     r0, =0xe0053409\n"
            "    movs    r1, #0\n"
            "    b       loc_e005351a\n"
            "loc_e005358c:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e005341c\n"
            "loc_e0053592:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004b4e0\n"
            "    movs    r2, #4\n"
            "    movs    r0, #0x92\n"
            "    add     r1, sp, #4\n"
            "    bl      _GetPropertyCase_FW\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00535b2\n"
            "    movs    r0, #0\n"
            "    movw    r2, #0x1c3\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e00535b2:\n"
            "    ldr     r1, =0x00037eac\n"
            "    ldr     r0, [sp, #4]\n"
            "    str     r0, [r1]\n"
            "    ldr.w   r0, [r7, #0xc0]\n"
            "    cbnz    r0, loc_e00535c2\n"
            "    bl      sub_e0035ea8\n"
            "loc_e00535c2:\n"
            "    bl      sub_e004b572\n"
            "    movs    r1, #0\n"
            "    mov     r0, r4\n"
            "    bl      sub_e00538fa\n"
            "    mov     r5, r0\n"
            "    bl      wait_until_remote_button_is_released\n"    // + remote hook
            "    ldr     r0, [sp, #8]\n"
            "    ubfx    r0, r0, #8, #8\n"
            "    cmp     r0, #6\n"
            "    beq     loc_e0053622\n"
            "    ldr     r6, =0xe004b275\n"
            "loc_e00535dc:\n"
            "    ldrh    r0, [r4, #0x18]\n"
            "    ldr.w   r8, =0xfffff3a0\n"
            "    cbz     r0, loc_e0053626\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e005363c\n"
            "    cmp     r0, #4\n"
            "    beq     loc_e0053626\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x210\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "    movs    r6, #0x1d\n"
            "loc_e00535fa:\n"
            "    ldrsh.w r0, [sp, #0xc]\n"
            "    cmp     r0, r8\n"
            "    beq     loc_e0053614\n"
            "    bl      sub_e0066cbe\n"
            "    cbz     r0, loc_e0053614\n"
            "    movs    r2, #1\n"
            "    movs    r1, #0\n"
            "    movs    r0, #0x46\n"
            "    bl      sub_e0299940\n"
            "    movs    r6, #1\n"
            "loc_e0053614:\n"
            "    bl      sub_e004b576\n"
            "    ldr.w   r8, =0xe0053409\n"
            "    lsls    r0, r6, #0x1f\n"
            "    beq     loc_e00536bc\n"
            "    b       loc_e0053730\n"
            "loc_e0053622:\n"
            "    ldr     r6, =0xe004b25f\n"
            "    b       loc_e00535dc\n"
            "loc_e0053626:\n"
            "    str     r5, [sp]\n"
            "    mov     r3, r6\n"
            "    ldr     r1, [r4, #0x64]\n"
            "    mov     r0, r4\n"
            "    ldr     r2, [sp, #4]\n"
            "    bl      sub_e004ae3e\n"
            "    mov     r6, r0\n"
            "    bl      sub_e043e5e2\n"
            "    b       loc_e00535fa\n"
            "loc_e005363c:\n"
            "    ldr.w   r0, [r7, #0xc0]\n"
            "    cbz     r0, loc_e005364e\n"
            "    movs    r0, #0\n"
            "    movw    r2, #0x1e7\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e005364e:\n"
            "    str     r5, [sp]\n"
            "    mov     r3, r6\n"
            "    ldr     r1, [r4, #0x64]\n"
            "    mov     r0, r4\n"
            "    ldr     r2, [sp, #4]\n"
            "    bl      sub_e004aeba\n"
            "    mov     r6, r0\n"
            "    ldrsh.w r0, [sp, #0xc]\n"
            "    cmp     r0, r8\n"
            "    beq     loc_e005366c\n"
            "    bl      sub_e0066cbe\n"
            "    cbnz    r0, loc_e0053676\n"
            "loc_e005366c:\n"
            "    movs    r2, #1\n"
            "    movs    r1, #0\n"
            "    movs    r0, #0x45\n"
            "    bl      sub_e0299940\n"
            "loc_e0053676:\n"
            "    lsls    r0, r6, #0x1f\n"
            "    bne     loc_e00535fa\n"
            "    ldr.w   r0, [r7, #0xfc]\n"
            "    cbz     r0, loc_e0053690\n"
            "    ldr     r1, [r4, #8]\n"
            "    ldr     r2, =0x00229618\n"
            "    ldr     r0, [r4, #0x64]\n"
            "    add.w   r1, r2, r1, lsl #2\n"
            "    str     r0, [r1, #-0x4]\n"
            "    b       loc_e00536b6\n"
            "loc_e0053690:\n"
            "    ldr     r0, =0xe0053409\n"
            "    movs    r1, #0\n"
            "    bl      sub_e0066cf0\n"
            "    movs    r1, #1\n"
            "    mov     r0, r4\n"
            "    bl      sub_e00538fa\n"
            "    mov     r5, r0\n"
            "    ldr     r0, [sp, #4]\n"
            "    bl      sub_e004b2fe\n"
            "    ldr     r1, [r4, #0x64]\n"
            "    mov     r3, r5\n"
            "    ldr     r2, [sp, #4]\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004af2c\n"
            "    mov     r6, r0\n"
            "loc_e00536b6:\n"
            "    bl      sub_e004ade6\n"
            "    b       loc_e00535fa\n"
            "loc_e00536bc:\n"
            "    ldr.w   r0, [r7, #0x104]\n"
            "    cbnz    r0, loc_e00536fc\n"
            "    ldr.w   r0, [r7, #0x1cc]\n"
            "    cbnz    r0, loc_e00536ce\n"
            "    ldr.w   r0, [r7, #0x1c8]\n"
            "    cbz     r0, loc_e00536e6\n"
            "loc_e00536ce:\n"
            "    ldrh.w  r0, [r7, #0x1ec]\n"
            "    cmp     r0, #1\n"
            "    bne     loc_e00536fc\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004baf4\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00536fc\n"
            "    movw    r2, #0x227\n"
            "    b       loc_e00536f4\n"
            "loc_e00536e6:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004baf4\n"
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e00536fc\n"
            "    movw    r2, #0x22a\n"
            "loc_e00536f4:\n"
            "    movs    r0, #0\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e00536fc:\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004b562\n"
            "    mov     r0, r4\n"
            "    bl      sub_e004b536\n"
            "    cmp     r5, r8\n"
            "    beq     loc_e0053730\n"
            "    bl      sub_e005157e\n"
            "    movw    sb, #0x23a\n"
            "    movs    r1, #4\n"
            "    ldr     r3, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    movw    r2, #0x3a98\n"
            "    str.w   sb, [sp]\n"
            "    bl      sub_e02f0954\n"
            "    cbz     r0, loc_e0053730\n"
            "    movs    r0, #0\n"
            "    mov     r2, sb\n"
            "    ldr     r1, =0xe005381c\n" //  *"SsStandardCaptureSeq.c"
            "    bl      _DebugAssert\n"
            "loc_e0053730:\n"
            "    bl      capt_seq_hook_raw_here\n"  // +
            "    ldrh    r0, [r7]\n"
            "    sub.w   r1, r0, #0x4000\n"
            "    subs    r1, #0x45\n"
            "    bne     loc_e0053744\n"
            "    movs    r0, #0\n"
            "    bl      sub_e0299fb0\n"
            "    bl      sub_e0299fa6\n"
            "loc_e0053744:\n"
            "    ldr.w   r0, [r7, #0x188]\n"
            "    cbz     r0, loc_e0053754\n"
            "    movs    r2, #1\n"
            "    movs    r1, #0\n"
            "    movs    r0, #0x46\n"
            "    bl      sub_e0299940\n"
            "loc_e0053754:\n"
            "    movs    r1, #2\n"
            "    mov     r2, r4\n"
            "    mov     r0, r6\n"
            "    bl      sub_e004ff4a\n"
            "    ldr     r0, [r7, #0x28]\n"
            "    cbz     r0, loc_e0053776\n"
            "    cmp     r5, r8\n"
            "    beq     loc_e005376a\n"
            "    movs    r1, #1\n"
            "    b       loc_e005376c\n"
            "loc_e005376a:\n"
            "    movs    r1, #0\n"
            "loc_e005376c:\n"
            "    mov     r2, r6\n"
            "    mov     r0, r4\n"
            "    bl      sub_e0053184\n"
            "    b       loc_e0053788\n"
            "loc_e0053776:\n"
            "    cmp     r5, r8\n"
            "    beq     loc_e005377e\n"
            "    movs    r1, #1\n"
            "    b       loc_e0053780\n"
            "loc_e005377e:\n"
            "    movs    r1, #0\n"
            "loc_e0053780:\n"
            "    mov     r2, r6\n"
            "    mov     r0, r4\n"
            "    bl      sub_e005313e\n"
            "loc_e0053788:\n"
            "    add     sp, #0x10\n"
            "    mov     r0, r6\n"
            "    pop.w   {r4, r5, r6, r7, r8, sb, sl, pc}\n"
);
}

/*************************************************************/
//** exp_drv_task @ 0xE02D0AD9, length=510 
void __attribute__((naked,noinline)) exp_drv_task() {
asm volatile (
            "    push.w  {r4, r5, r6, r7, r8, sb, sl, fp, lr}\n"
            "    mov.w   fp, #0\n"
            "    ldr.w   sl, =0x0000e1a8\n"
            "    sub     sp, #0x34\n"
            "    ldr.w   sb, =0xfffff400\n"
            "    movs    r7, #0x3e\n"
            "    ldr     r5, =0x000647b0\n"
            "    mov.w   r8, #1\n"
            "    str.w   fp, [sp, #0x24]\n"
            "loc_e02d0af6:\n"
            "    movs    r2, #0\n"
            "    ldr.w   r0, [sl, #0x20]\n"
            "    add     r1, sp, #0x14\n"
            "    mov     r4, sl\n"
            "    bl      sub_dffc9de0\n" // ReceiveMessageQueue
            "    ldr     r0, [sp, #0x14]\n"
            "    cmp     r7, #0x15\n"
            "    ldr     r0, [r0]\n"
            "    beq     loc_e02d0b20\n"
            "    cmp     r7, #0x16\n"
            "    beq     loc_e02d0b20\n"
            "    cmp     r7, #0x17\n"
            "    beq     loc_e02d0b20\n"
            "    cmp     r7, #0x18\n"
            "    beq     loc_e02d0b20\n"
            "    cmp     r7, #0x19\n"
            "    beq     loc_e02d0b20\n"
            "    cmp     r7, #0x1a\n"
            "    bne     loc_e02d0b48\n"
            "loc_e02d0b20:\n"
            "    cmp     r0, #0x15\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x16\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x17\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x18\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x19\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x1a\n"
            "    beq     loc_e02d0b50\n"
            "    cmp     r0, #0x3b\n"
            "    beq     loc_e02d0c26\n"
            "    cmp     r0, #0x36\n"
            "    beq     loc_e02d0b50\n"
            "    movs    r0, #0\n"
            "    add     r1, sp, #0x24\n"
            "    bl      sub_e02d0a94\n"
            "loc_e02d0b48:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldr     r0, [r0]\n"
            "    cmp     r0, #0x3b\n"
            "    beq     loc_e02d0c26\n"
            "loc_e02d0b50:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldr     r7, [r0]\n"
            "    cmp     r7, #0x3e\n"
            "    beq     loc_e02d0be2\n"
            "    cmp     r7, #0x3d\n"
            "    beq     loc_e02d0bf8\n"
            "    cmp     r7, #0x34\n"
            "    beq     loc_e02d0c00\n"
            "    cmp     r7, #0x35\n"
            "    beq     loc_e02d0c2c\n"
            "    cmp     r7, #0x36\n"
            "    beq     loc_e02d0c5c\n"
            "    cmp     r7, #0x3b\n"
            "    beq     loc_e02d0c26\n"
            "    cmp     r7, #0x3c\n"
            "    beq     loc_e02d0c5e\n"
            "    movs    r6, #1\n"
            "    cmp     r7, #0x13\n"
            "    beq     loc_e02d0c60\n"
            "    cmp     r7, #0x14\n"
            "    beq     loc_e02d0c60\n"
            "    cmp     r7, #0x15\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x16\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x17\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x18\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x19\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x1a\n"
            "    beq     loc_e02d0c62\n"
            "    cmp     r7, #0x2d\n"
            "    beq     loc_e02d0c64\n"
            "    cmp     r7, #0x2e\n"
            "    beq     loc_e02d0c64\n"
            "    adds    r1, r0, #4\n"
            "    add     r4, sp, #8\n"
            "    ldm     r1, {r1, r2, r3}\n"
            "    stm.w   r4, {r1, r2, r3}\n"
            "    ldr     r1, [r0]\n"
            "    cmp     r1, #0x34\n"
            "    bhs     loc_e02d0ca6\n"
            "    tbb     [pc, r1]\n" // (jumptable r1 52 elements)
            "branchtable_e02d0bae:\n"
            "    .byte((loc_e02d0d8a - branchtable_e02d0bae) / 2)\n" // (case 0)
            "    .byte((loc_e02d0d8a - branchtable_e02d0bae) / 2)\n" // (case 1)
            "    .byte((loc_e02d0d88 - branchtable_e02d0bae) / 2)\n" // (case 2)
            "    .byte((loc_e02d0d86 - branchtable_e02d0bae) / 2)\n" // (case 3)
            "    .byte((loc_e02d0d86 - branchtable_e02d0bae) / 2)\n" // (case 4)
            "    .byte((loc_e02d0d86 - branchtable_e02d0bae) / 2)\n" // (case 5)
            "    .byte((loc_e02d0d8a - branchtable_e02d0bae) / 2)\n" // (case 6)
            "    .byte((loc_e02d0d88 - branchtable_e02d0bae) / 2)\n" // (case 7)
            "    .byte((loc_e02d0d86 - branchtable_e02d0bae) / 2)\n" // (case 8)
            "    .byte((loc_e02d0d86 - branchtable_e02d0bae) / 2)\n" // (case 9)
            "    .byte((loc_e02d0d84 - branchtable_e02d0bae) / 2)\n" // (case 10)
            "    .byte((loc_e02d0d84 - branchtable_e02d0bae) / 2)\n" // (case 11)
            "    .byte((loc_e02d0d84 - branchtable_e02d0bae) / 2)\n" // (case 12)
            "    .byte((loc_e02d0d82 - branchtable_e02d0bae) / 2)\n" // (case 13)
            "    .byte((loc_e02d0d80 - branchtable_e02d0bae) / 2)\n" // (case 14)
            "    .byte((loc_e02d0d80 - branchtable_e02d0bae) / 2)\n" // (case 15)
            "    .byte((loc_e02d0d80 - branchtable_e02d0bae) / 2)\n" // (case 16)
            "    .byte((loc_e02d0d80 - branchtable_e02d0bae) / 2)\n" // (case 17)
            "    .byte((loc_e02d0d7e - branchtable_e02d0bae) / 2)\n" // (case 18)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 19)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 20)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 21)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 22)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 23)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 24)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 25)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 26)
            "    .byte((loc_e02d0d7c - branchtable_e02d0bae) / 2)\n" // (case 27)
            "    .byte((loc_e02d0d7a - branchtable_e02d0bae) / 2)\n" // (case 28)
            "    .byte((loc_e02d0d7a - branchtable_e02d0bae) / 2)\n" // (case 29)
            "    .byte((loc_e02d0d7a - branchtable_e02d0bae) / 2)\n" // (case 30)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 31)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 32)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 33)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 34)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 35)
            "    .byte((loc_e02d0d78 - branchtable_e02d0bae) / 2)\n" // (case 36)
            "    .byte((loc_e02d0d76 - branchtable_e02d0bae) / 2)\n" // (case 37)
            "    .byte((loc_e02d0d74 - branchtable_e02d0bae) / 2)\n" // (case 38)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 39)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 40)
            "    .byte((loc_e02d0d72 - branchtable_e02d0bae) / 2)\n" // (case 41)
            "    .byte((loc_e02d0d70 - branchtable_e02d0bae) / 2)\n" // (case 42)
            "    .byte((loc_e02d0d6e - branchtable_e02d0bae) / 2)\n" // (case 43)
            "    .byte((loc_e02d0d6e - branchtable_e02d0bae) / 2)\n" // (case 44)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 45)
            "    .byte((loc_e02d0ca6 - branchtable_e02d0bae) / 2)\n" // (case 46)
            "    .byte((loc_e02d0d6c - branchtable_e02d0bae) / 2)\n" // (case 47)
            "    .byte((loc_e02d0d6c - branchtable_e02d0bae) / 2)\n" // (case 48)
            "    .byte((loc_e02d0d6c - branchtable_e02d0bae) / 2)\n" // (case 49)
            "    .byte((loc_e02d0d6a - branchtable_e02d0bae) / 2)\n" // (case 50)
            "    .byte((loc_e02d0d6a - branchtable_e02d0bae) / 2)\n" // (case 51)
            ".align 1\n"
            "loc_e02d0be2:\n"
            "    bl      sub_e02d1bdc\n"
            "    ldr.w   r0, [sl, #0x1c]\n"
            "    movs    r1, #1\n"
            "    bl      _SetEventFlag\n"
            "    bl      _ExitTask\n"
            "    add     sp, #0x34\n"
            "    b       sub_e02d0814\n" //  return // jump to FW
            "loc_e02d0bf8:\n"
            "    ldrd    r1, r2, [r0, #0xac]\n"
            "    mov     r0, r2\n"
            "    b       loc_e02d0c28\n"
            "loc_e02d0c00:\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #0x80\n"
            "    bl      sub_dffc9966\n" // ClearEventFlag
            "    ldr     r0, =0xe02cc4b9\n"
            "    movs    r1, #0x80\n"
            "    bl      sub_e0061dd6\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #0x80\n"
            "    movw    r2, #0xbb8\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02d0cc2\n"
            "    movw    r2, #0x19e7\n"
            "    b       loc_e02d0c5a\n"
            "loc_e02d0c26:\n"
            "    b       loc_e02d0c78\n"
            "loc_e02d0c28:\n"
            "    blx     r1\n"
            "    b       loc_e02d1008\n"
            "loc_e02d0c2c:\n"
            "    add     r1, sp, #0x24\n"
            "    bl      sub_e02d0a94\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    mov.w   r6, #0x100\n"
            "    mov     r1, r6\n"
            "    bl      sub_dffc9966\n" // ClearEventFlag
            "    ldr     r0, =0xe02cc4c3\n"
            "    mov     r1, r6\n"
            "    bl      sub_e006279c\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movw    r2, #0xbb8\n"
            "    mov     r1, r6\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02d0cc2\n"
            "    movw    r2, #0x19f1\n"
            "loc_e02d0c5a:\n"
            "    b       loc_e02d0c66\n"
            "loc_e02d0c5c:\n"
            "    b       loc_e02d0c70\n"
            "loc_e02d0c5e:\n"
            "    b       loc_e02d0c86\n"
            "loc_e02d0c60:\n"
            "    b       loc_e02d0cca\n"
            "loc_e02d0c62:\n"
            "    b       loc_e02d0d04\n"
            "loc_e02d0c64:\n"
            "    b       loc_e02d0dc0\n"
            "loc_e02d0c66:\n"
            "    movs    r0, #0\n"
            "    ldr     r1, =0xe02d0db0\n" //  *"ExpDrv.c"
            "    bl      _DebugAssert\n"
            "    b       loc_e02d0cc2\n"
            "loc_e02d0c70:\n"
            "    add     r1, sp, #0x24\n"
            "    bl      sub_e02d0a94\n"
            "    b       loc_e02d0cc2\n"
            "loc_e02d0c78:\n"
            "    bl      sub_e00724de\n"
            "    bl      sub_e047005c\n"
            "    bl      sub_e043aff4\n"
            "    b       loc_e02d0cc2\n"
            "loc_e02d0c86:\n"
            "    bl      sub_e035bbe4\n"
            "    mov     r6, r0\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #4\n"
            "    bl      sub_dffc9966\n" // ClearEventFlag
            "    ldr     r2, =0xe02cc4d7\n"
            "    movs    r3, #4\n"
            "    mov     r1, r6\n"
            "    mov     r0, sb\n"
            "    str.w   r8, [sp]\n"
            "    bl      sub_e02d1c2e\n"
            "    b       loc_e02d0ca8\n"
            "loc_e02d0ca6:\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0ca8:\n"
            "    bl      sub_e00721e6\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #4\n"
            "    movw    r2, #0xbb8\n"
            "    bl      sub_dffc9830\n" // WaitForAnyEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02d0cc2\n"
            "    movw    r2, #0x1ae3\n"
            "    b       loc_e02d0c5a\n"
            "loc_e02d0cc2:\n"
            "    ldr     r1, [sp, #0x14]\n"
            "    ldrd    r1, r0, [r1, #0xac]\n"
            "    b       loc_e02d0c28\n"
            "loc_e02d0cca:\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    add     r4, sp, #8\n"
            "    add.w   r1, r1, r1, lsl #1\n"
            "    add.w   r1, r0, r1, lsl #2\n"
            "    subs    r1, #8\n"
            "    ldm     r1, {r1, r2, r3}\n"
            "    stm.w   r4, {r1, r2, r3}\n"
            "    bl      sub_e02cf23a\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldrd    r3, r2, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    adds    r0, r0, #4\n"
            "    blx     r3\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    bl      sub_e02d1df0\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldrd    r3, r2, [r0, #0xb4]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    b       loc_e02d0f34\n"
            "loc_e02d0d04:\n"
            "    add     r3, sp, #0x24\n"
            "    add     r2, sp, #0x28\n"
            "    add     r1, sp, #8\n"
            "    bl      sub_e02cf446\n"
            "    cmp     r0, #1\n"
            "    mov     r4, r0\n"
            "    beq     loc_e02d0d32\n"
            "    cmp     r4, #5\n"
            "    beq     loc_e02d0d32\n"
            "    cmp     r4, #2\n"
            "    beq     loc_e02d0d44\n"
            "    cmp     r4, #6\n"
            "    beq     loc_e02d0d44\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    mov     r2, r4\n"
            "    ldrd    ip, r3, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    adds    r0, r0, #4\n"
            "    blx     ip\n"
            "    b       loc_e02d0f38\n"
            "loc_e02d0d32:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    mov     r2, r4\n"
            "    ldrd    ip, r3, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    adds    r0, r0, #4\n"
            "    blx     ip\n"
            "    b       loc_e02d0d5e\n"
            "loc_e02d0d44:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    mov     r2, r4\n"
            "    ldrd    ip, r3, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    adds    r0, r0, #4\n"
            "    blx     ip\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    add     r2, sp, #0x28\n"
            "    add     r1, sp, #8\n"
            "    bl      sub_e02d0818\n"
            "loc_e02d0d5e:\n"
            "    ldr     r2, [sp, #0x24]\n"
            "    mov     r1, r4\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    bl      sub_e02d0a44\n"
            "    b       loc_e02d0f38\n"
            "loc_e02d0d6a:\n"
            "    b       loc_e02d0f14\n"
            "loc_e02d0d6c:\n"
            "    b       loc_e02d0f0e\n"
            "loc_e02d0d6e:\n"
            "    b       loc_e02d0f08\n"
            "loc_e02d0d70:\n"
            "    b       loc_e02d0ede\n"
            "loc_e02d0d72:\n"
            "    b       loc_e02d0eb4\n"
            "loc_e02d0d74:\n"
            "    b       loc_e02d0e8a\n"
            "loc_e02d0d76:\n"
            "    b       loc_e02d0e60\n"
            "loc_e02d0d78:\n"
            "    b       loc_e02d0e5a\n"
            "loc_e02d0d7a:\n"
            "    b       loc_e02d0e4c\n"
            "loc_e02d0d7c:\n"
            "    b       loc_e02d0e46\n"
            "loc_e02d0d7e:\n"
            "    b       loc_e02d0f26\n"
            "loc_e02d0d80:\n"
            "    b       loc_e02d0f20\n"
            "loc_e02d0d82:\n"
            "    b       loc_e02d0f1a\n"
            "loc_e02d0d84:\n"
            "    b       loc_e02d0e52\n"
            "loc_e02d0d86:\n"
            "    b       loc_e02d0e40\n"
            "loc_e02d0d88:\n"
            "    b       loc_e02d0e3a\n"
            "loc_e02d0d8a:\n"
            "    b       loc_e02d0e34\n"
            "loc_e02d0dc0:\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    add.w   ip, sp, #8\n"
            "    add.w   r1, r1, r1, lsl #1\n"
            "    add.w   r1, r0, r1, lsl #2\n"
            "    subs    r1, #8\n"
            "    ldm     r1, {r1, r2, r3}\n"
            "    stm.w   ip, {r1, r2, r3}\n"
            "    bl      sub_e02ce3b0\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldrd    r3, r2, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    adds    r0, r0, #4\n"
            "    blx     r3\n"
            "loc_e02d0dea:\n"
            "    bl      sub_e00724de\n"
            "loc_e02d0dee:\n"
            "    bl      sub_e047005c\n"
            "loc_e02d0df2:\n"
            "    bl      sub_e043aff4\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #2\n"
            "loc_e02d0dfa:\n"
            "    movw    r2, #0xbb8\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02d0e14\n"
            "loc_e02d0e06:\n"
            "    movw    r2, #0xe6d\n"
            "loc_e02d0e0a:\n"
            "    ldr     r1, =0xe02d0db0\n" //  *"ExpDrv.c"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "loc_e02d0e14:\n"
            "    ldr     r0, [r4, #0x1c]\n"
            "    movs    r1, #0x20\n"
            "    movw    r2, #0xbb8\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02d0e32\n"
            "    movw    r2, #0xe71\n"
            "    ldr     r1, =0xe02d0db0\n" //  *"ExpDrv.c"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "loc_e02d0e32:\n"
            "    b       loc_e02d0f38\n"
            "loc_e02d0e34:\n"
            "    bl      sub_e02ccc2e\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e3a:\n"
            "    bl      sub_e02ccecc\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e40:\n"
            "    bl      sub_e02cd0e4\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e46:\n"
            "    bl      sub_e02cd3a0\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e4c:\n"
            "    bl      sub_e02cd566\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e52:\n"
            "    bl      sub_e02cd9ee_my\n" // Patched (was sub_e02cd9ee)
            "    movs    r6, #0\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e5a:\n"
            "    bl      sub_e02cdb0c\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e60:\n"
            "    ldrh    r1, [r0, #4]\n"
            "    strh.w  r1, [sp, #8]\n"
            "    ldrh    r1, [r5, #2]\n"
            "    strh.w  r1, [sp, #0xa]\n"
            "    ldrh    r1, [r5, #4]\n"
            "    strh.w  r1, [sp, #0xc]\n"
            "    ldrh    r1, [r5, #6]\n"
            "    strh.w  r1, [sp, #0xe]\n"
            "    ldrh    r1, [r0, #0xc]\n"
            "    strh.w  r1, [sp, #0x10]\n"
            "    ldrh    r1, [r5, #0xa]\n"
            "    strh.w  r1, [sp, #0x12]\n"
            "    bl      sub_e02ce0da\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0e8a:\n"
            "    ldrh    r1, [r0, #4]\n"
            "    strh.w  r1, [sp, #8]\n"
            "    ldrh    r1, [r5, #2]\n"
            "    strh.w  r1, [sp, #0xa]\n"
            "    ldrh    r1, [r5, #4]\n"
            "    strh.w  r1, [sp, #0xc]\n"
            "    ldrh    r1, [r5, #6]\n"
            "    strh.w  r1, [sp, #0xe]\n"
            "    ldrh    r1, [r5, #8]\n"
            "    strh.w  r1, [sp, #0x10]\n"
            "    ldrh    r1, [r5, #0xa]\n"
            "    strh.w  r1, [sp, #0x12]\n"
            "    bl      sub_e02d1c78\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0eb4:\n"
            "    ldrh    r1, [r5]\n"
            "    strh.w  r1, [sp, #8]\n"
            "    ldrh    r1, [r0, #6]\n"
            "    strh.w  r1, [sp, #0xa]\n"
            "    ldrh    r1, [r5, #4]\n"
            "    strh.w  r1, [sp, #0xc]\n"
            "    ldrh    r1, [r5, #6]\n"
            "    strh.w  r1, [sp, #0xe]\n"
            "    ldrh    r1, [r5, #8]\n"
            "    strh.w  r1, [sp, #0x10]\n"
            "    ldrh    r1, [r5, #0xa]\n"
            "    strh.w  r1, [sp, #0x12]\n"
            "    bl      sub_e02d1d00\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0ede:\n"
            "    ldrh    r1, [r5]\n"
            "    strh.w  r1, [sp, #8]\n"
            "    ldrh    r1, [r5, #2]\n"
            "    strh.w  r1, [sp, #0xa]\n"
            "    ldrh    r1, [r5, #4]\n"
            "    strh.w  r1, [sp, #0xc]\n"
            "    ldrh    r1, [r5, #6]\n"
            "    strh.w  r1, [sp, #0xe]\n"
            "    ldrh    r1, [r0, #0xc]\n"
            "    strh.w  r1, [sp, #0x10]\n"
            "    ldrh    r1, [r5, #0xa]\n"
            "    strh.w  r1, [sp, #0x12]\n"
            "    bl      sub_e02d1d80\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f08:\n"
            "    bl      sub_e02ce196\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f0e:\n"
            "    bl      sub_e02ce7bc\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f14:\n"
            "    bl      sub_e02cebda\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f1a:\n"
            "    bl      sub_e02cedfc\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f20:\n"
            "    bl      sub_e02cef88\n"
            "    b       loc_e02d0f2a\n"
            "loc_e02d0f26:\n"
            "    bl      sub_e02cf092\n"
            "loc_e02d0f2a:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldrd    r3, r2, [r0, #0xac]\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "loc_e02d0f34:\n"
            "    adds    r0, r0, #4\n"
            "    blx     r3\n"
            "loc_e02d0f38:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    ldr     r0, [r0]\n"
            "    cmp     r0, #0x10\n"
            "    beq     loc_e02d0f8e\n"
            "    bgt     loc_e02d0f54\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e02d0f64\n"
            "    cmp     r0, #4\n"
            "    beq     loc_e02d0f64\n"
            "    cmp     r0, #0xe\n"
            "    beq     loc_e02d0f8e\n"
            "    cmp     r0, #0xf\n"
            "    bne     loc_e02d0fa4\n"
            "    b       loc_e02d0f64\n"
            "loc_e02d0f54:\n"
            "    cmp     r0, #0x11\n"
            "    beq     loc_e02d0f64\n"
            "    cmp     r0, #0x14\n"
            "    beq     loc_e02d0f64\n"
            "    cmp     r0, #0x1a\n"
            "    beq     loc_e02d0f64\n"
            "    cmp     r0, #0x1d\n"
            "    bne     loc_e02d0fa4\n"
            "loc_e02d0f64:\n"
            "    ldrsh.w r0, [r5]\n"
            "    cmp     r0, sb\n"
            "    beq     loc_e02d0f74\n"
            "    ldrsh.w r1, [r5, #8]\n"
            "    cmp     r1, sb\n"
            "    bne     loc_e02d0f84\n"
            "loc_e02d0f74:\n"
            "    add     r0, sp, #0x18\n"
            "    bl      sub_e0368074\n" // get_current_exp
            "    ldrh.w  r0, [sp, #0x18]\n"
            "    strh.w  r0, [sp, #8]\n"
            "    b       loc_e02d0f9c\n"
            "loc_e02d0f84:\n"
            "    strh.w  r0, [sp, #8]\n"
            "    strh.w  r1, [sp, #0x10]\n"
            "    b       loc_e02d0fa4\n"
            "loc_e02d0f8e:\n"
            "    ldrsh.w r0, [r5, #8]\n"
            "    cmp     r0, sb\n"
            "    bne     loc_e02d0fa0\n"
            "    add     r0, sp, #0x18\n"
            "    bl      sub_e0368074\n" // get_current_exp
            "loc_e02d0f9c:\n"
            "    ldrh.w  r0, [sp, #0x20]\n"
            "loc_e02d0fa0:\n"
            "    strh.w  r0, [sp, #0x10]\n"
            "loc_e02d0fa4:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    cbz     r6, loc_e02d0fe0\n"
            "    ldr.w   r1, [r0, #0x94]\n"
            "    ldr     r2, [sp, #8]\n"
            "    add.w   r1, r1, r1, lsl #1\n"
            "    add.w   r0, r0, r1, lsl #2\n"
            "    ldr     r1, =0x00064784\n"
            "    str     r2, [r1, #0x2c]\n"
            "    ldr     r2, [sp, #0xc]\n"
            "    str     r2, [r1, #0x30]\n"
            "    ldr     r2, [sp, #0x10]\n"
            "    str     r2, [r1, #0x34]\n"
            "    ldr     r2, [sp, #8]\n"
            "    str     r2, [r1, #0x38]\n"
            "    ldr     r2, [sp, #0xc]\n"
            "    str     r2, [r1, #0x3c]\n"
            "    ldr     r2, [sp, #0x10]\n"
            "    str     r2, [r1, #0x40]\n"
            "    ldr     r2, [r0, #-0x8]\n"
            "    str     r2, [r1, #0x44]\n"
            "    ldr     r2, [r0, #-0x4]\n"
            "    str     r2, [r1, #0x48]\n"
            "    ldr     r0, [r0]\n"
            "    str     r0, [r1, #0x4c]\n"
            "    b       loc_e02d1008\n"
            "loc_e02d0fe0:\n"
            "    ldr     r0, [r0]\n"
            "    mov     r4, r8\n"
            "    cmp     r0, #0xc\n"
            "    beq     loc_e02d1010\n"
            "    movs    r3, #1\n"
            "    mov     r2, r3\n"
            "    mov     r1, r3\n"
            "    mov     r0, r3\n"
            "    str     r4, [sp]\n"
            "    str     r4, [sp, #4]\n"
            "    bl      sub_e02cca7a\n"
            "    movs    r3, #1\n"
            "    str     r4, [sp]\n"
            "    mov     r2, r3\n"
            "    mov     r1, r3\n"
            "    mov     r0, r3\n"
            "    str     r4, [sp, #4]\n"
            "loc_e02d1004:\n"
            "    bl      sub_e02ccbac\n"
            "loc_e02d1008:\n"
            "    ldr     r0, [sp, #0x14]\n"
            "    bl      sub_e02d1bdc\n"
            "    b       loc_e02d0af6\n"
            "loc_e02d1010:\n"
            "    movs    r3, #1\n"
            "    movs    r0, #0\n"
            "    strd    fp, r4, [sp]\n"
            "    mov     r6, fp\n"
            "    mov     r2, r3\n"
            "    mov     r1, r3\n"
            "    bl      sub_e02cca7a\n"
            "    movs    r3, #1\n"
            "    movs    r0, #0\n"
            "    strd    r6, r4, [sp]\n"
            "    mov     r2, r3\n"
            "    mov     r1, r3\n"
            "    b       loc_e02d1004\n"
);
}

/*************************************************************/
//** sub_e02cd9ee_my @ 0xE02CD9EF, length=104 
void __attribute__((naked,noinline)) sub_e02cd9ee_my() {
asm volatile (
            "    push.w  {r4, r5, r6, r7, r8, lr}\n"
            "    mov     r4, r0\n"
            "    ldr     r7, =0x0000e1a8\n"
            "    movs    r1, #0x3e\n"
            "    ldr     r0, [r7, #0x1c]\n"
            "    bl      sub_dffc9966\n" // ClearEventFlag
            "    ldrsh.w r0, [r4, #4]\n"
            "    movs    r2, #0\n"
            "    movs    r3, #1\n"
            "    mov     r1, r2\n"
            "    bl      sub_e02cc518\n"
            "    mov     r6, r0\n"
            "    ldrsh.w r0, [r4, #6]\n"
            "    bl      sub_e02cc6b0\n"
            "    ldrsh.w r0, [r4, #8]\n"
            "    bl      sub_e02cc6f6\n"
            "    ldrsh.w r0, [r4, #0xa]\n"
            "    bl      sub_e02cc73c\n"
            "    ldrsh.w r0, [r4, #0xc]\n"
            "    movs    r1, #0\n"
            "    bl      sub_e02cc782\n"
            "    mov     r5, r0\n"
            "    ldr     r0, [r4]\n"
            "    ldr.w   r8, =0x000647c8\n"
            "    cmp     r0, #0xc\n"
            "    beq     loc_e02cda42\n"
            "    cmp     r6, #1\n"
            "    beq     loc_e02cdaa6\n"
            "    b       loc_e02cda46\n"
            "loc_e02cda42:\n"
            "    movs    r6, #0\n"
            "    mov     r5, r6\n"
            "loc_e02cda46:\n"
            "    ldrh.w  r0, [r8]\n"
            "    strh    r0, [r4, #4]\n"
            "loc_e02cda4c:\n"
            "    cmp     r5, #1\n"
            "    beq     loc_e02cdaba\n"
            "    ldrh.w  r0, [r8, #8]\n"
            "loc_e02cda54:\n"
            "    strh    r0, [r4, #0xc]\n"
            "    ldrsh.w r0, [r4, #6]\n"
            "    bl      sub_e0072240_my\n" // Patched (was sub_e0072240)
            "    ldrsh.w r0, [r4, #8]\n"
            "    movs    r1, #1\n"
            "    bl      sub_e043ad64\n"
            "    movs    r1, #0\n"
            "    add.w   r0, r4, #8\n"
//            "    b       loc_e02cda90\n" // -
            "loc_e02cda90:\n"
            "    bl      sub_e043add0\n"
            "    ldrsh.w r0, [r4, #0xe]\n"
            "    bl      sub_e007e50e\n"
            "    cmp     r6, #1\n"
            "    movw    r4, #0xbb8\n"
            "    beq     loc_e02cdac8\n"
            "    b       loc_e02cdae4\n"
            "loc_e02cdaa6:\n"
            "    ldrsh.w r0, [r4, #4]\n"
            "    movs    r2, #2\n"
            "    ldr     r1, =0xe02cc4af\n"
            "    bl      sub_e0061fe6\n"
            "    strh    r0, [r4, #4]\n"
            "    movs    r0, #0\n"
            "    str     r0, [r7, #0x28]\n"
            "    b       loc_e02cda4c\n"
            "loc_e02cdaba:\n"
            "    ldrsh.w r0, [r4, #0xc]\n"
            "    movs    r2, #0x20\n"
            "    ldr     r1, =0xe02cc503\n"
            "    bl      sub_e02d1c4e\n"
            "    b       loc_e02cda54\n"
            "loc_e02cdac8:\n"
            "    ldr     r0, [r7, #0x1c]\n"
            "    movs    r1, #2\n"
            "    mov     r2, r4\n"
            "loc_e02cdace:\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02cdae4\n"
            "loc_e02cdad6:\n"
            "    movw    r2, #0x99e\n"
            "    ldr     r1, =0xe02ccd3c\n" //  *"ExpDrv.c"
            "loc_e02cdade:\n"
            "    movs    r0, #0\n"
            "    bl      _DebugAssert\n"
            "loc_e02cdae4:\n"
            "    cmp     r5, #1\n"
            "loc_e02cdae6:\n"
            "    bne     loc_e02cdb08\n" //  return
            "    ldr     r0, [r7, #0x1c]\n"
            "    movs    r1, #0x20\n"
            "    mov     r2, r4\n"
            "    bl      sub_dffc98b6\n" // WaitForAllEventFlag
            "    lsls    r0, r0, #0x1f\n"
            "    beq     loc_e02cdb08\n" //  return
            "    movw    r2, #0x9a4\n"
            "    ldr     r1, =0xe02ccd3c\n" //  *"ExpDrv.c"
            "    pop.w   {r4, r5, r6, r7, r8, lr}\n"
            "    movs    r0, #0\n"
            "    b.w     _DebugAssert\n"
            "loc_e02cdb08:\n"
            "    pop.w   {r4, r5, r6, r7, r8, pc}\n"
);
}

/*************************************************************/
//** sub_e0072240_my @ 0xE0072241, length=24 jfw
void __attribute__((naked,noinline)) sub_e0072240_my() {
asm volatile (
            "    push    {r4, r5, r6, lr}\n"
            "    mov     r4, r0\n"
            "    ldr     r5, =0x0000f728\n"
            "    ldr     r0, [r5, #4]\n"
            "    cmp     r0, #1\n"
            "    beq     loc_e0072258\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x172\n"
            "    ldr     r1, =0xe007228c\n" //  *"Shutter.c"
            "    bl      _DebugAssert\n"
            "loc_e0072258:\n"
            "    ldr     r0, =0xfffff400\n"
            "    cmp     r4, r0\n"
            "    bne     loc_e0072262\n"
            "    ldrsh.w r4, [r5, #2]\n"
            "loc_e0072262:\n"
            "    cmp     r4, r0\n"
            "    strh    r4, [r5, #2]\n"
            "    bne     loc_e0072274\n"
            "    movs    r0, #0\n"
            "    mov.w   r2, #0x178\n"
            "    ldr     r1, =0xe007228c\n" //  *"Shutter.c"
            "    bl      _DebugAssert\n"
            "loc_e0072274:\n"
            "    mov     r0, r4\n"
            "    bl      apex2us\n" // Patched (was _apex2us)
            "    ldr     pc, =0xe007227b\n" // Continue in firmware
);
}
